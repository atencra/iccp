function [fradata] = iccp_ccpairs_select_plot_fra_params(fradata)
% iccpairs_ccpairs_select_max_plot_fra_params FRA params for synchronous spikes
%
% [fradata] = iccp_ccpairs_select_plot_fra_params;
% iccp_ccpairs_select_plot_fra_params(fradata)
% --------------------------------------------------------
...
%     The call:
% 
%         [fradata] = iccpairs_ccpairs_select_plot_fra_params;
% 
...
%     The call:
% 
%         iccpairs_ccpairs_select_plot_fra_params(fradata);
% 
...
d = dir('*-fracmb-pairs-isi-fixed-ccpairs-select.mat');


for n = 1:length(d)

    ccfile = d(n).name;
    s = load(ccfile, 'ccpairs_select');
    ccpairs = s.ccpairs_select;

    if ( length(ccpairs) > 0 )

        index = findstr(ccfile, '-isi-fixed-ccpairs-select.mat');
...
% fprintf('ccpairs: %s site %.0f chan %.0f stim %s\n', exp, site, chan, stim);
% ccpairs(ii).model1
% ccpairs(ii).model2
