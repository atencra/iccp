function [rfdata] = iccp_ccpairs_select_max_plot_strf_params(rfdata)
% iccpairs_ccpairs_select_max_plot_strf_params STRF params for synchronous spikes
%
% [rfdata] = iccp_ccpairs_select_max_plot_strf_params;
% iccp_ccpairs_select_max_plot_strf_params(rfdata)
% --------------------------------------------------------
...
if ( nargin == 0 )
   [position, cc, fr, pli] = get_pairs_strf_params;

    rfdata.cc = cc;
    rfdata.position = position;
    rfdata.fr = fr;
    rfdata.pli = pli;
end
...
function [position, cc, fr, pli] = get_pairs_strf_params
    ...
    if ( length(ccpairs) > 0 )
        ...
        for ii = 1:length(ccpairs)
            ...
            if ( index1 > 0 && index2 > 0 )
                ...
                fr = [fr; params(index1).w0 params(index2).w0];
                pli = [pli; params(index1).pli params(index2).pli];
            end
