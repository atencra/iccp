function [fradata] = iccpairs_ccpairs_select_max_plot_fra_params(fradata)
% iccpairs_ccpairs_select_max_plot_fra_params FRA params for synchronous spikes
%
% [fradata] = iccpairs_ccpairs_select_max_plot_fra_params;
% iccpairs_ccpairs_select_max_plot_fra_params(fradata)
% --------------------------------------------------------
...
% 
%     *-fracmb-pairs-params.mat     
%     *-fracmb-pairs-resptype.mat     
%     *-fracmb-pairs-isi-fixed-ccpairs-select-max.mat   
% 
...
if ( nargin == 0 )
   [position, fr, cf, bw, bwlog, Q, latency, resptype] = get_pairs_fra_params;

    fradata.position = position;
    fradata.fr = fr;
    fradata.cf = cf;
    fradata.bw = bw;
    fradata.bwlog = bwlog;
    fradata.q = Q;
    fradata.latency = latency;
    fradata.resptype = resptype;
end
...
% vs_plot_pairs_fra_fr_scatter_hist(fradata.fr);

% vs_plot_pairs_fra_cf_scatter_hist(fradata.cf);

vs_plot_pairs_fra_bw_scatter_hist(fradata.bw);

% vs_plot_pairs_fra_bw_diff_rand(fradata.bw);

vs_plot_pairs_fra_q_scatter_hist(fradata.q);

return;


% vs_plot_pairs_fra_q_diff_rand(fradata.q)


% vs_plot_pairs_fra_latency_scatter_hist(fradata.latency);
% 
...
function [position, fr, cf, bw, bwlog, Q, latency, resptype] = get_pairs_fra_params
...
d = dir('*-fracmb-pairs-isi-fixed-ccpairs-select-max.mat');
...
for n = 1:length(d)
    ...
    if ( length(ccpairs) > 0 )
        ...
        paramsfile = sprintf('%s-params.mat', prefix);
        s = load(paramsfile, 'params');
        params = s.params;

        respfile = sprintf('%s-resptype.mat', prefix);
        s = load(respfile, 'rt');
        rt = s.rt;
        ...
        for ii = 1:length(ccpairs)
            ...
            % Find/save response type values
            index1 = iccpairs_find_fra_params_element(rt, exp, ...
                site, chan, model1, stim);

            index2 = iccpairs_find_fra_params_element(rt, exp, ...
                site, chan, model2, stim);

            resptype = [resptype; rt(index1).ne_nb_ne_total rt(index2).ne_nb_ne_total];



            % Find where the parameter values are for each neuron
            index1 = iccpairs_find_fra_params_element(params, exp, ...
                site, chan, model1, stim);

            index2 = iccpairs_find_fra_params_element(params, exp, ...
                site, chan, model2, stim);
...
set(gcf,'position', [1003 648 361 551]);
print_mfilename(mfilename);
