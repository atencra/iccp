function [rbarpop, dtpop, infopop, rbarpairs, dtpairs, infopairs] = ...
   iccp_plot_info_calc_pop(rbarpop, dtpop, infopop, rbarpairs, dtpairs, infopairs)
% iccp_plot_info_calc_pop Information parameters for population data
%
...
%
%
% Data for these plots are stored in: iccpairs-information-pop-pairs.mat
%
%
...
[absc, ord] = iccp_randomize_columns(rbarpairs(:,1), rbarpairs(:,2));
...
[rpop_med, rpop_ci, pval] = ...
    iccp_pairwise_corr_rand_test(rbarpairs(:,1), rbarpairs(:,2), logtransform);


fprintf('\n');
fprintf('Rbar pairs - randomized\n');
...
[absc, ord] = iccp_randomize_columns(dtpairs(:,1), dtpairs(:,2));
...
logtransform = 0;
[rpop_med, rpop_ci, pval] = ...
    iccp_pairwise_corr_rand_test(dtpairs(:,1), dtpairs(:,2), logtransform);

fprintf('\n');
fprintf('DT pairs - randomized\n');
fprintf('Pairwise Randomization: r=%.4f,p=%.4f\n', rpop_med,pval);
fprintf('Confidence Intervals: [%.4f, %.4f]\n', rpop_ci(1), rpop_ci(2));
...
[absc, ord] = iccp_randomize_columns(infopairs(:,1), infopairs(:,2));
...
logtransform = 1;
[rpop_med, rpop_ci, pval] = ...
    iccp_pairwise_corr_rand_test(infopairs(:,1), infopairs(:,2), logtransform);

fprintf('\n');
fprintf('Info pairs - randomized\n');
fprintf('Pairwise Randomization: r=%.4f,p=%.4f\n', rpop_med,pval);
fprintf('Confidence Intervals: [%.4f, %.4f]\n', rpop_ci(1), rpop_ci(2));
